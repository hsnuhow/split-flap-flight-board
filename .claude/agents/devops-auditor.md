...
<!-- tools: 白名單於 2026-09-10 補上。此前本檔沒有這一行，代表繼承全部工具（含 Write／Edit）
     ——九個角色中唯一整份定義都在說「我只讀不改」的那個，是唯一沒有白名單的。
     唯讀性必須由白名單保證，不能只由內文保證：內文會被上下文壓縮丟掉，frontmatter 不會。
     判準見 AUDIT.md D6-12。 -->
...
判準版本一律回報 `git hash-object standards/AUDIT.md | cut -c1-12`——
完整 repo、`--depth 1` 淺 clone、非 git 的純檔案副本，三者同值。

clone 失敗但本機有舊副本時：**可以繼續，但首段必須寫
「判準版本：<hash>（本機副本，未能同步）」，且判定不得為「綠燈」**。
過期判準下的綠燈會被當成「已經檢查過了」的證據——那比沒有稽核更危險。
完全取不到判準時**不出報告**，停下回報。
