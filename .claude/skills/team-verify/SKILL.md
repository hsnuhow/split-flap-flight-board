...
開始前先讀本專案 `CLAUDE.md` 的部署章節，確認以下八個語意角色各自對應哪個口令
（語意角色的完整定義見 OpDev/standards/DEPLOYMENT.md 第二節）：

| 語意角色 | 本專案的口令 |
|---|---|
| 預覽（部署但不接流量） | 讀 CLAUDE.md |
| 中介（部署到 staging，**僅雙軌專案有**） | 讀 CLAUDE.md |
| 上線（部署到最終對外環境） | 讀 CLAUDE.md |
| 還原（從穩定點回復） | 讀 CLAUDE.md |
| 開 PR | 讀 CLAUDE.md |
| 合併進 `main` | 讀 CLAUDE.md |
| 推送非 `main` 分支到遠端（**有 `develop` 等整合分支的專案才有**） | 讀 CLAUDE.md |
| 撤銷／急停（收回已給的授權） | 讀 CLAUDE.md |

> ⚠️ **急停口令屬本表，不是紅線。** 它是收回授權的動作，不是禁止事項；
> 列進 `CLAUDE.md` 的紅線節會讓抽取工具與稽核都判錯。
...
- **循環自治口令**（若本專案 CLAUDE.md 有定義，例如 `核准循環自治：核准部署：stage`）：在該**指定階段範圍內**自行 開發+執行+部署到**非最終環境**+複驗，循環到驗收達成或連續 2 輪無新問題。
...
6. **收斂判斷**：回到 4 複驗。連續 2 輪無新 **HIGH／MEDIUM** → 視為收斂。
