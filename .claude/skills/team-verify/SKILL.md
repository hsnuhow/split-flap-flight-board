...
## 🔴 口令用字全站相同，現場事實以本專案 CLAUDE.md 為準

**本 SKILL 不發明任何口令。** 口令用字**全站相同**，正典口令表見
`OpDev/standards/DEPLOYMENT.md` 第二節；專案 `CLAUDE.md` 第 1 節是它的副本
（只有上線口令那一列依軌別二擇一）。

開始前仍要先讀本專案 `CLAUDE.md` 的部署章節，確認以下八個語意角色：

| 語意角色 | 本專案的口令 |
|---|---|
| 預覽（部署但不接流量） | `核准部署：preview` |
| 中介（部署到 staging，**僅雙軌專案有**） | `核准部署：stage` |
| 上線（部署到最終對外環境） | `核准部署：正式`（單軌）／`核准部署：production`（雙軌）——看本專案軌別 |
| 還原（從穩定點回復） | `核准部署：還原` |
| 開 PR | `核准推送：PR` |
| 合併進 `main` | `核准推送：merge` |
...
**本表與該專案 `CLAUDE.md` 分歧時，以 `CLAUDE.md` 為現場事實照它走，同時回報分歧**
（那代表某一邊沒跟上正典表）。

> ⚠️ **急停口令屬本表，不是紅線。** 它是收回授權的動作，不是禁止事項；
> 列進 `CLAUDE.md` 的紅線節會讓抽取工具與稽核都判錯。

**若 `CLAUDE.md` 缺某個角色的口令，那是該專案的缺陷（`AUDIT.md` D6-2 判 HIGH）——回報，
不要自行代入正典用字就動手。**
...
- **無口令 / 只給開發類口令（`核准開發`、`核准修正` 等）**：跑 find→fix→review→**再驗證（編譯/type-check/邏輯）**，**停在部署前**，回報「請下 `核准部署：preview`」（`preview` 單軌雙軌都有、一律必須，不需要條件分支）。
- **循環自治口令**（正典族，**一律必須定義**；例如 `核准循環自治：核准部署：preview`——`preview` 是單軌雙軌都有的階段，`stage` 只有雙軌有）：在該**指定階段範圍內**自行 開發+執行+部署到**非最終環境**+複驗，循環到驗收達成或連續 2 輪無新問題。
  ...
  - 範圍模糊就停下問，不自我擴張階段。
- **全鏈自治口令**（終點是上線口令的那一支，**一律必須定義**；能不能真的跑完見該專案 `DEPLOYMENT.md` 第四節）：煞車條件以該專案 `CLAUDE.md` 為準。
